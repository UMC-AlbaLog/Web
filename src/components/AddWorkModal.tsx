        ...
        <div className="space-y-6 overflow-y-auto flex-1 scrollbar-hide">
          ...
          <div className="relative">
            ...
            <div className="flex gap-2">
              <input type="text" placeholder="가게 이름을 입력하세요" value={states.searchKeyword} onChange={(e) => setters.setSearchKeyword(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && actions.handleSearch(states.searchKeyword)} className="flex-1 px-4 py-3 bg-gray-50 rounded-xl text-sm border-2 border-transparent focus:border-yellow-400 outline-none transition-all" />
            ...
            {states.isSearching && (
              <div className="absolute z-20 w-full bg-white border border-gray-100 rounded-2xl mt-2 shadow-2xl max-h-52 overflow-y-auto">
                {states.searchResults.length > 0 ? (
                  <ul>{states.searchResults.map((place, i) => (<li key={i} onClick={() => actions.handleSelectPlace(place)} className="p-4 hover:bg-yellow-50 cursor-pointer border-b last:border-0 transition-colors"><p className="text-sm font-bold text-gray-800">{place.place_name}</p><p className="text-[10px] text-gray-400 mt-1">{place.address_name}</p></li>))}</ul>
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="flex gap-4">
              <div className="flex-1"><label className="text-sm font-bold block mb-2 text-gray-700">시작 시간</label><input type="time" value={states.startTime} onChange={(e) => setters.setStartTime(e.target.value)} className="w-full px-4 py-3 bg-gray-50 rounded-xl text-sm" /></div>
              <div className="flex-1"><label className="text-sm font-bold block mb-2 text-gray-700">종료 시간</label><input type="time" value={states.endTime} onChange={(e) => setters.setEndTime(e.target.value)} className="w-full px-4 py-3 bg-gray-50 rounded-xl text-sm" /></div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center justify-between bg-gray-50 px-4 py-3 rounded-xl border border-gray-100">
              <div className="flex items-center gap-1 font-bold"><input type="number" value={states.pay} onChange={(e) => setters.setPay(Number(e.target.value))} className="bg-transparent w-20 outline-none" /><span>원</span></div>
              <span className="text-xs text-yellow-600 font-extrabold italic">예상 {states.totalPay.toLocaleString()}원</span>
            </div>
          </div>
          <div><label className="text-sm font-bold block mb-2 text-gray-700">4. 메모</label><textarea value={states.memo} onChange={(e) => setters.setMemo(e.target.value)} className="w-full px-4 py-3 bg-gray-50 rounded-xl text-sm h-20 resize-none outline-none focus:border-yellow-400" /></div>
        </div>
        <button onClick={handleSubmit} className="w-full bg-yellow-400 text-black py-4 rounded-2xl font-extrabold mt-8 hover:bg-yellow-500 shadow-lg">일정 추가하기</button>
      </div>
    </div>
  );
};
