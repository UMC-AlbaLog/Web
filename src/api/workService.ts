...
export const workService = {
  ...
  getTodaySummary: async () => {
    const res = await api.get(`/works/today/summary?t=${Date.now()}`);
    return res.data.success || res.data;
  },

  // 오늘의 근무 리스트
  getTodayWorkLogs: async () => {
    const res = await api.get(`/api/work-logs/today?t=${Date.now()}`);
  ...
  checkOut: async (workLogId: string) => {
    const res = await api.patch(`/api/work-logs/${workLogId}/check-out`);
    return res.data.success || res.data;
  }
};